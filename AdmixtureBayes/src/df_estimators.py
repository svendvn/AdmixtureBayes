...
def variance_mean_based(sample_of_matrices):
    ...
    r=mean_wishart.shape[0]
    var_rom_mean_wishart=(np.square(mean_wishart)+np.diag(mean_wishart)).T+np.diag(mean_wishart)
    ...
    return minimize(penalty_function, r, bounds=[(r,None)]).x[0]
