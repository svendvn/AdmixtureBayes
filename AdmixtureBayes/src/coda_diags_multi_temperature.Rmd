...
#summs_df=summs_df[1:(nrow(summs_df)-4),]
library('coda')
```
...
all_plots=function(df){
  mcmcobj=mcmc(df)
  rd=raftery.diag(mcmcobj)
  resm=rd$resmatrix
  crosscorr.plot(mcmcobj)
  for(i in 1:ncol(df)){
    v=colnames(df)[i]
    df2=as.data.frame(df[floor(nrow(df)/10):nrow(df),v])
    #print(df2)
    colnames(df2) <- c(v)
    mcmcobj=mcmc(df2)
    plot(mcmcobj)
...
df2=apply(dfa[,as.character(variables_to_plot_normally)],c(1,2),as.numeric)
all_plots(df2)
```
...
for(variable in variables_to_plot_normally){
  if(variable %in% colnames(sim_df)){
    val=as.numeric(as.character(summs_df[which(summs_df$summary==variable),4]))
    print(val)
    print(hist_numbers2(df2[,variable], val, name=variable))
  }
}
```
...
```{r}
split_up=function(s){
  return(strsplit(s, split = ';', fixed = T)[[1]][1])
}
dfa$topology=sapply(as.character(dfa$tree), split_up)
```

```{r}
sort(table(dfa$topology), decreasing=T)[1:10]
```

```{r}
x1=subset(dfa, proposal_type=='deladmix')$mhr
x2=subset(dfa, proposal_type=='addadmix')$mhr
...
```{r}
branch_lengths=function(x, pattern){
  identifier_branch_props=strsplit(x, split=';', fixed =T)[[1]]
  identifier=identifier_branch_props[1]
  #print(identifier==pattern)
  if(identifier==pattern){
    branch_strings=strsplit(identifier_branch_props[2], '-', fixed= T)[[1]]
    return(as.numeric(branch_strings))
  }
  return(numeric(0))
}

pat=names(sort(table(dfa$topology), decreasing=T))[1]
print(pat)
res=matrix(0, nrow=0, ncol=20)
for(i in 1:nrow(dfa)){
  ad=branch_lengths(as.character(dfa$tree[i]), pat)
  if(length(ad)>0){
    #print(ad)
    res=rbind(res,ad)
  }
...
apply(res,2,quantile, p=c(0.025,0.5,0.975))
```

```{r}
for( c in colnames(dfa)){
  if(endsWith(c,'adap_param')){
    plot(na.omit(dfa[,c]), type='l', ylab=c)
  }
}
```
