...
all_plots=function(df){
  ...
  for(i in 1:ncol(df)){
    v=colnames(df)[i]
    df2=as.data.frame(df[floor(nrow(df)/2):nrow(df),v])
    #print(df2)
    colnames(df2) <- c(v)
    mcmcobj=mcmc(df2)
    plot(mcmcobj)
...
branch_lengths=function(x, pattern){
  identifier_branch_props=strsplit(x, split=';')[[1]]
  identifier=identifier_branch_props[1]
  if(identifier==pattern){
    branch_strings=strsplit(identifier_branch_props[2], '-')[[1]]
