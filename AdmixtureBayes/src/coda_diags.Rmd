...
hist_numbers=function(x, true){
  fd=hist(c(x,true), plot=F)
  ad=hist(x, col=rgb(1,0,0,0.5), breaks=fd$breaks, freq=F, ylim=c(0,max(fd$density)*1.5))
...
for(variable in variables_to_plot_normally){
  if(variable %in% colnames(sim_df)){
    print(hist_numbers(df2[,variable], sim_df[,variable]))
  }
}
```
